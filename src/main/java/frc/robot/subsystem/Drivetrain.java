...
public class Drivetrain implements SubSystem {
  ...
  /**
   * This method is to stop power in the robot Not a hard stop unless brake is on
   * the motors (hard stop is underneath)
   */
  public void stop() {
    setPower(0, 0);
  ...
  /**
   * Congrats you found hard stop :) This method hardstops the robot
   */
  ...
  public void hardStop() {
    ...
    // in last years code (2018) Jiah spelt timed timne... is this some wierd
    // convention i dont know, or was it a mistake?
  ...
  public void followGyro(double power, double gyroTarget) {
    // proportionally drives in the direction of a gyro heading, turning to face the
    // right direction
    double currentGyroAngle = getGyroAngle() % 360;
    double gyroPowerAdjustment = 0;
    double gyroGain = 0.05;

    // Calculates how much to turn based on the current heading and the target
    // heading
  ...
  /**
   * Performs a curve turn with the robot to the left (if left is true to the
   * target angle. An error handler is implemented to interupt the turn after a
   ...
   * logger.info("curveTurn [" + targetAngle + ":" + left + "]"); double turnPower
   * = 0.3;
   * 
   * SmartDashboard.putNumber("Mr. Curveturns target angle is: ", targetAngle);
   * SmartDashboard.putBoolean("Mr. Curveturn going left?" , left);
   * 
   * //reset mr. gyro mr_gyro.reset(); // do not calibrate as this cause the
   * apocalypse :( while(Math.abs(getGyroAngle()) < (Math.abs(targetAngle))){
   * if(left){ setPower(turnPower, 0); } else { //must want to turn right
   * setPower(0, turnPower); } }
   * 
   * logger.info("Curve turn success"); }
   * 
   * public void turn(double turnAngle, boolean left){ logger.info("turn [" +
   * turnAngle + ":" + left + "]"); double turnPower = 0.3;
   * 
   * SmartDashboard.putNumber("turn.targetAngle", turnAngle);
   * SmartDashboard.putBoolean("turn.left", left);
   * 
   * // reset gyro sensor to zero mr_gyro.reset(); // do not calibrate as this
   * will stop the world and make the gyro crazy
   * 
   * long startTime = System.currentTimeMillis(); long timeTaken = 0;
   * 
   * while ( (Math.abs(getModGyroAngle()) < (Math.abs(turnAngle-10))) &&
   * (timeTaken < 5000) ) { if (left) { setPower(turnPower, -turnPower); } else {
   * // must want to turn right setPower(-turnPower, turnPower); }
   * 
